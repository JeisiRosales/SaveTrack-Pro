...
export const DashboardView: React.FC = () => {
    ...
    const {
        ...
        fixedExpenses, fixedExpensesSummary, nextFixedExpense, statusCounts,
        ...
        <main className="flex-1 p-5 lg:p-8 relative overflow-x-hidden">
            ...
            {error ? (
                ...
                <div className="space-y-6 max-w-[1280px]">
                    ...
                    {/* Sección de Gastos Fijos (Compromisos Mensuales) */}
                    <div>
                        <div className="flex items-center justify-between mb-4">
                            <div>
                                <h3 className="text-sm font-bold text-[var(--foreground)]">Compromisos Mensuales</h3>
                                <p className="text-[10px] text-[var(--muted)] font-medium">
                                    Estado de tus gastos fijos del mes
                                </p>
                            </div>
                            {/* Ajustar Link según donde se gestionen */}
                        ...
                        {(!fixedExpenses || fixedExpenses.length === 0) ? (
                            <div className="bg-[var(--card)] border border-[var(--card-border)] rounded-2xl p-6 text-center">
                                <p className="text-[var(--muted)] text-sm font-medium">
                                    No tienes registrado ningún gasto fijo.
                                </p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {/* Total de Gastos Fijos */}
                                <div className="bg-[var(--card)] border border-[var(--card-border)] rounded-2xl p-5 hover:border-indigo-500/40 transition-all">
                                    <div className="flex items-start justify-between mb-3">
                                    ...
                                    <p className="text-2xl font-black text-[var(--foreground)] leading-none mb-1">
                                        {fmt(fixedExpensesSummary?.totalMonthly || 0)}
                                    </p>
                                    <p className="text-[10px] text-[var(--muted)] font-medium">
                                        Suma de todos tus compromisos del mes
                                    </p>
                                </div>

                                {/* Gasto Más Próximo */}
                                <div className="bg-[var(--card)] border border-[var(--card-border)] rounded-2xl p-5 hover:border-indigo-500/40 transition-all flex flex-col justify-between">
                                    <div>
                                        <div className="flex items-start justify-between mb-3">
                                            <p className="text-[10px] font-black text-[var(--muted)] uppercase tracking-wider">
                                                Próximo a Pagar
                                            </p>
                                            <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${nextFixedExpense?.status === 'OVERDUE' ? 'bg-rose-500/10' : nextFixedExpense?.status === 'PENDING' ? 'bg-amber-500/10' : 'bg-emerald-500/10'}`}>
                                                {nextFixedExpense?.status === 'OVERDUE' ? <Target className="w-3.5 h-3.5 text-rose-500" />
                                                    : nextFixedExpense?.status === 'PENDING' ? <Target className="w-3.5 h-3.5 text-amber-500" />
                                        ...
                                        {nextFixedExpense ? (
                                            <>
                                                <p className="text-lg font-black text-[var(--foreground)] truncate leading-none mb-1">
                                                    {nextFixedExpense.name}
                                                </p>
                                                <p className="text-xs font-bold text-rose-500 bg-rose-500/10 px-2 py-0.5 rounded inline-block mt-1">
                                                    Monto: {fmt(nextFixedExpense.remainingAmount ?? 0)}
                                                </p>
                                            </>
                                        ) : (
                                    ...
                                    <p className="text-[10px] text-[var(--muted)] font-medium mt-3">
                                        {nextFixedExpense ? `Vence el día ${nextFixedExpense.billing_day} del mes` : 'No hay pagos inminentes'}
                                    </p>
                                </div>

                                {/* Distribución por Estado */}
                                <div className="bg-[var(--card)] border border-[var(--card-border)] rounded-2xl p-5 flex flex-col justify-between">
                                    <p className="text-[10px] font-black text-[var(--muted)] uppercase tracking-wider mb-3">
                                        Distribución de Pagos
                                    </p>
                                    <div className="space-y-2 flex-col justify-center gap-1 flex-1">
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs font-medium text-[var(--muted)]">Pagados</span>
                                            <span className="text-xs font-bold text-emerald-500 bg-emerald-500/10 px-2 rounded-md">
                                                {statusCounts.PAID}
                                            </span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs font-medium text-[var(--muted)]">Pendientes</span>
                                            <span className="text-xs font-bold text-amber-500 bg-amber-500/10 px-2 rounded-md">
                                                {statusCounts.PENDING + statusCounts.PARTIAL}
                                            </span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs font-medium text-[var(--muted)]">Atrasados</span>
                                            <span className="text-xs font-bold text-rose-500 bg-rose-500/10 px-2 rounded-md">
                                                {statusCounts.OVERDUE}
                                            </span>
                                        </div>
                                        <div className="flex items-center justify-between pt-2 border-t border-[var(--card-border)]">
                                            <span className="text-xs font-bold text-[var(--foreground)]">Total de Gastos Fijos</span>
